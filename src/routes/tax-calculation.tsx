...
export const Route = createFileRoute('/tax-calculation')({
...
function RouteComponent() {
  ...
  const calculateTax = (income: number): { tax: number; taxableIncome: number; deductible: number; steps: string[] } => {
    if (income <= 0) return { tax: 0, taxableIncome: 0, deductible: 0, steps: [] }
    ...
    // )
    const applicableBracket = taxBrackets.find(bracket =>
      bracket.max ? (Math.ceil(income) >= bracket.min && Math.ceil(income) <= bracket.max) : (income) >= bracket.min
    )
    if (applicableBracket) {
      ...
      steps.push(`Gross Tax: ${income.toLocaleString()} × ${applicableBracket.rate / 100} = ${grossTax.toFixed(2)} Birr`)
      steps.push(`Deductible: ${totalDeductible.toLocaleString()} Birr`)
      steps.push(`Final Tax: ${grossTax.toFixed(2)} - ${totalDeductible.toLocaleString()} = ${totalTax.toFixed(2)} Birr`)
    }
  ...
  const calculateGrossIncome = (netIncome: number): number => {
    ...
    for (let i = 0; i < taxBrackets.length; i++) {
      ...
      const netAtNextMin = nextBracket ? calculateNetIncome(nextBracket.min) : Infinity;
  ...
  const structuredData = {
    ...
    "description": "Free online Ethiopian income tax calculator with pension calculations. Calculate tax from gross or net income with detailed breakdowns.",
  ...
  return (
    <>
      ...
      <div style={{ display: 'none' }}>
        <title>Ethiopian Tax Calculator 2024 - Free Income Tax & Pension Calculator</title>
        <meta name="description" content="Free Ethiopian tax calculator for 2024. Calculate income tax, pension, and net salary with detailed breakdowns. Supports both gross to net and net to gross calculations." />
        <meta name="keywords" content="Ethiopian tax calculator, income tax calculator, tax bracket calculator, pension calculator, salary calculator, Ethiopia tax 2024, gross to net salary" />
        <meta name="author" content="FormulaLab" />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content="Ethiopian Tax Calculator 2024 - Free Income Tax & Pension Calculator" />
        <meta property="og:description" content="Calculate your Ethiopian income tax and pension with our free online calculator. Get detailed breakdowns and tax bracket information." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://formulalab.com/tax-calculation" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Ethiopian Tax Calculator 2024" />
        <meta name="twitter:description" content="Free Ethiopian tax calculator with detailed breakdowns and tax bracket information." />
      ...
      <div className="max-w-4xl mx-auto p-2 space-y-6 overflow-auto">
        <header className="text-center">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Ethiopian Tax Calculator 2024</h1>
          <p className="text-sm sm:text-base text-gray-600">Free online calculator for Ethiopian employment income tax and pension calculations</p>
        </header>

        {/* Tax Calculator */}
        <main className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4">Ethiopian Income Tax Calculator 2024</h2>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                ...
                <div className="flex bg-gray-100 rounded-lg p-1 w-full sm:w-auto">
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${calculationMode === 'gross'
                  ...
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${calculationMode === 'net'
                      ? 'bg-white text-gray-900 shadow-sm ring-2 ring-blue-500'
                      : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                      }`}
                  >
            ...
            {/* Income Input */}
            <div className="mb-6">
              <label htmlFor="monthlyIncome" className="block text-sm font-medium text-gray-700 mb-3">
                Monthly {calculationMode === 'gross' ? 'Gross' : 'Net'} Income (Birr)
              </label>
              <div className="relative">
                <input
                  type="number"
                  id="monthlyIncome"
                  value={calculationMode === 'gross' ? (monthlyIncome || '') : (monthlyNetIncome || '')}
                  onChange={(e) => calculationMode === 'gross' ? handleMonthlyIncomeChange(e.target.value) : handleMonthlyNetIncomeChange(e.target.value)}
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                <div className="text-sm text-blue-600 font-medium">Annual {calculationMode === 'gross' ? 'Gross' : 'Net'} Income</div>
                <div className="text-lg font-bold text-blue-900">
                  {calculationMode === 'gross' ? annualIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : annualNetIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Birr
                </div>
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg border border-blue-200">
                  <div className="text-xs text-blue-600 font-medium mb-1">Gross Income</div>
                  <div className="text-lg font-bold text-blue-900">{monthlyIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                  <div className="text-xs text-blue-600">Birr</div>
                </div>

                <div className="bg-gradient-to-br from-red-50 to-red-100 p-4 rounded-lg border border-red-200">
                  <div className="text-xs text-red-600 font-medium mb-1">Tax</div>
                  <div className="text-lg font-bold text-red-900">{monthlyTaxCalculation.tax.toFixed(2)}</div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200">
                  ...
                  <div className="text-lg font-bold text-orange-900">{monthlyPension.toFixed(2)}</div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                  ...
                  <div className="text-lg font-bold text-green-900">{monthlyNetIncome.toFixed(2)}</div>
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                <div className="text-sm text-gray-700 text-center">
                  <span className="font-medium">{monthlyIncome.toFixed(2)}</span> - <span className="text-red-600 font-medium">{monthlyTaxCalculation.tax.toFixed(2)}</span> - <span className="text-orange-600 font-medium">{monthlyPension.toFixed(2)}</span> = <span className="text-green-600 font-bold">{monthlyNetIncome.toFixed(2)} Birr</span>
                </div>
              </div>
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <div className="mt-3 pt-3 border-t border-blue-200">
                    <div className="text-sm text-blue-700 flex items-center">
                      ...
                      <span><strong>Pension:</strong> {monthlyIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} × 7% = {monthlyPension.toFixed(2)} Birr</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-3 rounded-lg border border-blue-200">
                  <div className="text-xs text-blue-600 font-medium mb-1">Gross Annual</div>
                  <div className="text-base font-bold text-blue-900">{annualIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                  <div className="text-xs text-blue-600">Birr</div>
                </div>

                <div className="bg-gradient-to-br from-red-50 to-red-100 p-3 rounded-lg border border-red-200">
                  <div className="text-xs text-red-600 font-medium mb-1">Annual Tax</div>
                  <div className="text-base font-bold text-red-900">{annualTaxCalculation.tax.toFixed(2)}</div>
                  <div className="text-xs text-red-600">Birr</div>
                </div>

                <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-3 rounded-lg border border-orange-200">
                  <div className="text-xs text-orange-600 font-medium mb-1">Annual Pension</div>
                  <div className="text-base font-bold text-orange-900">{annualPension.toFixed(2)}</div>
                  <div className="text-xs text-orange-600">Birr</div>
                </div>

                <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 rounded-lg border border-green-200">
                  <div className="text-xs text-green-600 font-medium mb-1">Net Annual</div>
                  <div className="text-base font-bold text-green-900">{annualNetIncome.toFixed(2)}</div>
                  <div className="text-xs text-green-600">Birr</div>
                </div>
              </div>
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <div className="mt-3 pt-3 border-t border-blue-200">
                    <div className="text-sm text-blue-700 flex items-center">
                      ...
                      <span><strong>Pension:</strong> {annualIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} × 7% = {annualPension.toFixed(2)} Birr</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 sm:p-6">
        <div className="flex">
          ...
          <div className="ml-4">
            <h3 className="text-base font-medium text-blue-800 mb-3">How the calculation works</h3>
            <div className="text-sm text-blue-700 space-y-2">
              <p>• <strong>Tax Formula:</strong> Tax = (Gross Income × Tax Rate) - Deductible Amount</p>
              <p>• <strong>Pension Formula:</strong> Pension = Gross Income × 7%</p>
              <p>• <strong>Net Income Formula:</strong> Net = Gross Income - Tax - Pension</p>
              <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                <p className="font-medium text-blue-800 mb-2">Example for 10,001 Birr Gross:</p>
                <div className="space-y-1 text-sm">
                  <p>• Gross Tax = 10,001 × 30% = 3,000.30 Birr</p>
                  <p>• Final Tax = 3,000.30 - 1,350 = 1,650.30 Birr</p>
                  <p>• Pension = 10,001 × 7% = 700.07 Birr</p>
                  <p>• Net Income = 10,001 - 1,650.30 - 700.07 = 7,650.63 Birr</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="px-4 sm:px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-b">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Tax Brackets & Deductibles</h2>
        </div>
                  ...
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {bracket.rate > 0 ? (
                      <span className="font-mono text-xs">
                        (Income × {bracket.rate}%) - {bracket.deductible.toLocaleString()}
                      </span>
                    ) : (
                      <span className="text-green-600 font-medium">No tax</span>
                    )}
                  </td>
                </tr>
              ))}
